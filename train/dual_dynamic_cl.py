...
from utils.dual_cl.loss_dyn_new import DualBranchLoss
...
@hydra.main(config_path="../configs", config_name="train/dynamic", version_base="1.3")
def main(cfg):
    ...
    if rank == 0:
        ...
        logger.info("=" * 60)
        logger.info("DualCPS + Morphology-Adaptive Dynamic Slice Patch CL")
    ...
    if rank == 0:
        ...
        if cl_cfg and cl_cfg.get("enable", False):
            logger.info("Dynamic Slice Patch CL: enabled")
            logger.info(f"CL weight: {cl_cfg.get('weight')}")
            logger.info(f"canonical_patch_size: {cl_cfg.get('canonical_patch_size')}")
            logger.info(f"search_size: {cl_cfg.get('search_size')}")
            logger.info(f"patch range: {cl_cfg.get('min_patch_size')} - {cl_cfg.get('max_patch_size')}")
        else:
            logger.info("Dynamic Slice Patch CL: disabled")
