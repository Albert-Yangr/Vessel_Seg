...
class ComponentAdaptivePatchContrastiveLoss(nn.Module):
    ...
    def compute_macro_micro_loss(self, v_feats_list, v_masks_list, b_feats_list, b_masks_list):
        ...
        if v_macro is not None:
            ...
            pos = torch.exp(torch.matmul(v_norm, self.vessel_proto) / self.temperature)
            neg = torch.exp(torch.matmul(v_norm, self.bg_proto) / self.temperature)
        ...
        if b_macro is not None:
            ...
            pos = torch.exp(torch.matmul(b_norm, self.bg_proto) / self.temperature)
            neg = torch.exp(torch.matmul(b_norm, self.vessel_proto) / self.temperature)
            macro_loss = macro_loss + (-torch.log(pos / (pos + neg + 1e-8)).mean())
            valid_macro += 1
...
class DualBranchLoss(nn.Module):
    ...
    def forward(self, preds_l, mask_l, preds_u, feats_l, feats_u, current_epoch, img_l=None):
        ...
        # ==========================================
        with torch.no_grad():
            # 基础 CPS 逻辑：概率大于 0.5 即视为伪标签，不进行任何高级置信度过滤
            pseudo_1 = (torch.sigmoid(pred1_u) > 0.5).float()
            pseudo_2 = (torch.sigmoid(pred2_u) > 0.5).float()

        # 直接传入 pseudo_loss_fn 互相监督 (不带 mask)
        loss_ps_1 = self.pseudo_loss_fn(pred1_u, pseudo_2)
        loss_ps_2 = self.pseudo_loss_fn(pred2_u, pseudo_1)
        ...
        if self.enable_cl and feats_l is not None:
            ...
            if current_epoch >= self.warmup:
                # 🌟 隔离保护机制：虽然伪标签Loss不过滤，但对比学习的特征提取必须过滤
                # 否则双分支产生分歧的脏特征会导致对比学习坍塌。仅此处计算 reliable_mask
                with torch.no_grad():
                    diff = torch.abs(torch.sigmoid(pred1_u) - torch.sigmoid(pred2_u))
