...
class PulmonaryAreaAdaptivePatchContrastiveLoss(nn.Module):
    ...
    def compute_macro_micro_loss(self, v_feats_list, v_masks_list, b_feats_list, b_masks_list):
        macro_loss, micro_loss = torch.tensor(0.0, device=self.vessel_proto.device), torch.tensor(0.0,
        ...
        if v_macro is not None:
            ...
            pos, neg = torch.exp(torch.matmul(v_norm, self.vessel_proto) / self.temperature), torch.exp(
                torch.matmul(v_norm, self.bg_proto) / self.temperature)
            macro_loss = macro_loss + (-torch.log(pos / (pos + neg + 1e-8)).mean())
            valid_macro += 1
        ...
        if b_macro is not None:
            ...
            pos, neg = torch.exp(torch.matmul(b_norm, self.bg_proto) / self.temperature), torch.exp(
                torch.matmul(b_norm, self.vessel_proto) / self.temperature)
            macro_loss = macro_loss + (-torch.log(pos / (pos + neg + 1e-8)).mean())
            valid_macro += 1
        ...
        if v_feats_list:
            ...
            if keep.any():
                ...
                pos = torch.exp(torch.matmul(anchor, self.vessel_proto) / self.temperature)
                ...
                neg = (torch.exp(torch.bmm(anchor.detach().unsqueeze(1), F.normalize(f_keep, dim=1)).squeeze(
                    1) / self.temperature) * near_bg).sum(dim=1) / (near_bg.sum(dim=1) + 1e-8)
...
class DualBranchLoss(nn.Module):
    ...
    def forward(self, preds_l, mask_l, preds_u, feats_l, feats_u, current_epoch, img_l=None):
        ...
        # with torch.no_grad():
        #     pseudo_1 = (torch.sigmoid(pred1_u) > 0.5).float()
        #     pseudo_2 = (torch.sigmoid(pred2_u) > 0.5).float()
        #
        # loss_ps = 0.5 * (self.pseudo_loss_fn(pred1_u, pseudo_2) + self.pseudo_loss_fn(pred2_u, pseudo_1))
        ...
        if self.enable_cl and feats_l is not None:
            ...
            if current_epoch >= self.warmup:
                ...
                p_mask_1, p_mask_2 = pseudo_1.clone(), pseudo_2.clone()
                p_mask_1[reliable_mask == 0] = 255
                p_mask_2[reliable_mask == 0] = 255
                loss_cl += 0.5 * (
                            self.cl_loss_fn(feats_u[0], p_mask_1, False, False) + self.cl_loss_fn(feats_u[1], p_mask_2,
                                                                                                  False, False))
