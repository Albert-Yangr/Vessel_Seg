...
class DualBranchLoss(nn.Module):
    ...
    def forward(self, preds_l, mask_l, preds_u, current_epoch, img_l=None):
        ...
        # ==========================================
        with torch.no_grad():
            # 先将 Logits 转化为 0~1 的概率，然后以 0.5 为界二值化
            pseudo_1 = (torch.sigmoid(pred1_u) > 0.5).float()
            pseudo_2 = (torch.sigmoid(pred2_u) > 0.5).float()
        # with torch.no_grad():
        #     pseudo_1 = torch.argmax(pred1_u, dim=1, keepdim=True).float()  # [cite: 14]
        #     pseudo_2 = torch.argmax(pred2_u, dim=1, keepdim=True).float()  # [cite: 14]
