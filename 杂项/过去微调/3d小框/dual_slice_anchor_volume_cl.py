...
from utils.dual_cl.loss_slice_anchor_volume import DualBranchLoss
...
@hydra.main(config_path="../../../configs", config_name="train/slice_anchor_volume_cl", version_base="1.3")
def main(cfg):
    ...
    if rank == 0:
        ...
        logger.info("=" * 60)
        logger.info("DualCPS + 2D Slice Patch CL + Slice-Anchored 3D Volume CL")
    ...
    dual_loss = DualBranchLoss(
        ...
        volume_cl_cfg=cfg.loss_configs.get("volume_contrastive", None),
    ...
    if rank == 0:
        ...
        logger.info(f"2D slice CL enabled: {dual_loss.enable_cl}")
        logger.info(f"3D slice-anchored volume CL enabled: {dual_loss.enable_volume_cl}")
