...
# 修改输出文件夹名称，避免与双分支结果冲突
OUTPUT_FOLDER = "/home/yangrui/Project/Base-model/local_results/feature_analysis/" + dataset + "_SingleBranch_FullVolume"

# ⚠️ 注意：请将这里的路径替换为你真实的【单分支】模型权重路径
CKPT_PATHS = {
    "Single_Label": "/home/yangrui/Project/Base-model/local_results/checkpoints/CAS2023/CAS2023-refile/随机-全标签90/Stage2-79-0.8673.ckpt",
    "Single_Slice": "/home/yangrui/Project/Base-model/local_results/checkpoints/CAS2023/CAS2023-refile/随机-全切片90/Stage2-97-0.8480.ckpt"
}
...
def load_model(cfg, ckpt_path):
    """单分支模型的权重加载逻辑，与 infer_test.py 保持一致"""
    ...
    with torch.no_grad():
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            for name, ckpt in CKPT_PATHS.items():
                ...
                # 单分支主干网络同样拥有 upsamples[2], [3], [4] 结构
